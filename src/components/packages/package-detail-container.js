...
export default class PackageDetailContainer extends Component {
      ...
      .then(response => {
        if (response.data != []) {
          this.setState({ package_details: response.data },()=>{
            console.log("after getting data"+JSON.stringify(this.state.package_details,null,4))
          });
        }
      })
  ...
  render() {
    return (
      <React.Fragment>
        <PackageDetailComponent packageState={this.state} showAvailability={this.showAvailability} />
      </React.Fragment>
    );
  }
}
